...
function App() {
  ...
  const getData = async () => {
    const items = await axios.get(`/api/v1/allTodos`);
    if (items.data) {
  ...
  const addTodo = async (title) => {
    if (title.length > 30) {
      ...
      const { data } = await axios.post(`/api/v1/addTodo`, { title }, config);
      alert(data);
      getData();
    }
  };
  const deleteTodo = async (title) => {
    const config = { headers: { "Content-Type": "application/json" } };
    const { data } = await axios.delete(`/api/v1/deleteTodo/${title}`, config);
    alert(data);
    getData();
  };
  ...
  const updateTodo = async (title) => {
    const config = { headers: { "Content-Type": "application/json" } };
    const { data } = await axios.put(`/api/v1/updateTodo`, { title }, config);
    alert(data);
    getData();
  };
  const applyFilter = (e) => {
